...
- Removed the `store_raw_payloads` config field: it was declared but never read anywhere (dead configuration that never took effect). Existing `config.json` files containing the key keep loading (pydantic ignores unknown fields).
- `get_profile` no longer returns the plaintext `user_id` to the model; it now returns `account_id_masked`.
- Account-id masking is unified between the MCP server and the CLI via a shared `auth.mask_account_id`: a fixed six asterisks plus only the last 2 characters (inputs of length ≤ 2 are fully masked, empty values return `None`). The server previously kept the first 3 and last 2 characters with length-preserving asterisks, which leaked the id length and disagreed with the CLI output.
- CSV exports escape formula injection: string cells whose first non-whitespace character is `=`, `+`, `-`, `@` (leading spaces/Tabs/CR are stripped before the check) are prefixed with a single quote. Strings that parse as numbers (including negatives and decimals, e.g. `-3.5`) are no longer escaped, because the irreversible quote prefix would corrupt them on re-import.
- After a successful Xiaomi login the rotated `passToken` is written back to the keyring (previously it lived only in memory, so the next start failed with the stale token); a keyring write failure logs a warning without interrupting the sync.
- `doctor` warns when a non-default database path is in use (`--db` or `MI_FITNESS_DB_PATH`): on Windows the tool does not tighten ACLs on custom locations, so the user must verify permissions themselves (e.g. with `icacls`).
- The README privacy sections now warn that `query_*` tool results flow through the MCP client into the cloud LLM behind it, and that the server must only be wired to a local client over local stdio — never to a remote or hosted agent.

### Fixed

- Daily-activity steps no longer double-count when two sources (e.g. phone and band) report the same minute: records are merged per (local date, minute timestamp) taking the larger value, and the suppressed overlap is logged (fixes #11 — synced step counts ran tens to hundreds of steps above the app on dual-device days; measured +73/+208 steps on real data before the fix).
- Sync no longer stalls permanently on one bad record: `_sync_range` isolates per-record failures — a record that fails insert/validation increments `skipped` (previously stuck at 0 forever) and is logged in a per-range `bad_records` list (capped at 20 entries, error text truncated to 200 chars, no sensitive payload values), while the remaining records continue and the sync-state watermark still advances using the last successful record's timestamp. Chunk-level (adapter) failures keep the existing abort-and-report behaviour.
- Cloud adapter `iter_*` generators skip and count records whose `time` field is missing, unparsable, or earlier than year 2000, instead of collapsing ids to `..._0` and timestamps to 1970; an aggregate warning reports the skipped count per data type.
- Upserts now refresh all mutable columns: `body_measurements` also updates `bone_mass_kg` / `visceral_fat_score` / `basal_metabolism_kcal` / `metabolic_age`, `workouts` also updates `end_at` / `activity_type` / `avg_pace_sec_per_km` / `max_pace_sec_per_km` / `total_steps`, and `sleep_sessions` also updates `start_at` / `end_at` / `is_nap`.
...
- Body-measurement records with a missing or zero `weight` are skipped instead of crashing the generator on the `weight_kg > 0` validation (same class of bug as the earlier `bmi=0` fix).
- Leftover `shkyyy18/mi-bridge` repository references replaced with `shkyyy18/mi_fitness_data_bridge` in `server.json`, `pyproject.toml`, both READMEs, and the issue-template config (the `mi-bridge` server nickname in the MCP client config examples is intentionally kept).
- README.en.md "External contributions" statement corrected: two outside PRs have been merged (#3 docs, #7 feature) and #10 is under review.
...
- Added the own copyright line `Copyright (c) 2026 shkyyy18 and contributors` to the NOTICE block of `LICENSE` for the AGPL-3.0-only portion.
